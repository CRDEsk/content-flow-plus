              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                {stats.map((stat, index) => {
                  ...
                  return (
                    <div
                      key={`${timeView}-${index}`}
                      className="relative overflow-hidden rounded-xl sm:rounded-2xl bg-gradient-to-br from-zinc-900/50 to-zinc-950/50 p-3 sm:p-4 lg:p-6 border border-zinc-800/50 transition-all duration-200"
                      style={{ WebkitTransform: 'translateZ(0)', transform: 'translateZ(0)' }}
                    >
                      <div className="relative z-10">
                        <div className="flex items-start justify-between mb-2 sm:mb-3 lg:mb-4">
                          <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-lg sm:rounded-xl bg-zinc-800/50 flex items-center justify-center flex-shrink-0">
                            <Icon className="h-4 w-4 sm:h-5 sm:w-5 lg:h-6 lg:w-6 text-primary" />
                          </div>
                          <div className={`flex items-center gap-0.5 sm:gap-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-medium flex-shrink-0 ${
                  ...
                  <div className="absolute inset-0 flex items-end justify-between gap-2">
                    {chartData.map((value, index) => (
                      <div
                        key={`${timeView}-${index}`}
                        style={{ height: `${value}%` }}
                        className="relative flex-1 bg-gradient-to-t from-primary via-primary/80 to-primary/60 rounded-t-lg transition-all duration-200"
                      >
                      </div>
                    ))}
                  </div>
            ...
            <div className="rounded-xl sm:rounded-2xl bg-gradient-to-br from-zinc-900/50 to-zinc-950/50 p-4 sm:p-6 border border-zinc-800/50" style={{ WebkitTransform: 'translateZ(0)' }}>
              ...
              <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                {topClients.map((client, index) => (
                  <div
                    key={index}
                    className="group flex items-center justify-between p-2 sm:p-3 rounded-lg sm:rounded-xl transition-all duration-200"
                  >
              ...
              <button
                className="w-full px-3 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl bg-zinc-800/50 hover:bg-zinc-800 text-xs sm:text-sm font-medium text-foreground transition-colors border border-zinc-700/50 hover:border-zinc-600 active:scale-95"
              >
